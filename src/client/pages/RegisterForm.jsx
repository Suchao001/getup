...
const RegisterForm = () => {
	...
	const handleRegister = async (event) => {
		...
		try {
			const response = await axios.post(`${HostName}/api/user/register`, { username, password });
		...
		<Container component="main" maxWidth="xs" sx={{ minHeight: '100vh', padding: 3 }}>
			...
			<Paper elevation={3} sx={{ padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center', borderRadius: '10px' }}>
				...
				<form onSubmit={handleRegister} style={{ width: '100%', marginTop: '16px' }}>
					<label htmlFor="username" className='font1'>Username</label>
					<TextField
						label="Username"
						variant="outlined"
						size="small"
						...
						margin="normal"
					/>
					<label htmlFor="password" className='font1'>Password</label>
					<TextField
						label="Password"
						type="password"
						variant="outlined"
						...
						margin="normal"
					/>
					<label htmlFor="confirmPassword" className='font1'>Confirm Password</label>
					<TextField
