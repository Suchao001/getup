...
const HabitRecommend = ({ fetchHabits }) => {
  ...
  const fetchRecommendCategory = async () => {
    try {
      const res = await axios.get(`${HostName}/api/HabitRecommendCategory`, {
        withCredentials: true,
      });
      setRecommendCategory(Array.isArray(res.data) ? res.data : []);
    } catch (error) {
      console.error(error);
  ...
  const fetchRecommendHabit = async (id) => {
    try {
      const res = await axios.get(`${HostName}/api/HabitRecommendHabit/${id}`, {
        withCredentials: true,
      });
      setRecommendHabit(Array.isArray(res.data) ? res.data : []);
    } catch (error) {
      console.error(error);
      setRecommendHabit([]);
    }
  ...
  const RecommendHabit = ({
    ...
    return (
      <div
        className="recommend-habit"
        style={{ backgroundColor: recommendHabit.color }}
        onClick={() => handleClick(recommendHabit.id)}
      >
    ...
    <>
      ...
      <CustomModal open={open} onClose={handleClose}>
        <div className="recommend-card-container">
          {Array.isArray(recommendCategory) && recommendCategory.length > 0 ? (
      ...
      <NestedCustomModal open={openHabit} onClose={handleCloseHabit}>
        ...
        <div className="recommend-habit-container">
          {Array.isArray(recommendHabit) && recommendHabit.length > 0 ? (
            ...
            <p>No habit recommendations available</p>
          )}
        </div>
      </NestedCustomModal>
      <CustomModal
        title="Habit Recommend"
        open={openHabitForm}
