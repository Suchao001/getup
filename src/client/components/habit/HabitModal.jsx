  ...
  const handleDelete = async () => {
    try {
      ...
      const res = await axios.delete(
        `${HostName}/api/habits/delete/${habit.id}`,
          ...
          <Paper sx={modalStyle} className="habit-modal">
            ...
            <Box mb={4} className="habit-header">
              <Box display="flex" alignItems="center" gap={2} mb={1}>
                ...
                {habit.point && (
                  <Chip
                    label={`${habit.point} point${habit.point > 1 ? "s" : ""}`}
                    color="primary"
                    variant="filled"
                ...
                {["Morning", "Afternoon", "Evening", "Anytime"].map((time) => (
                  <Grid item key={time}>
                    <Chip
                      ...
                      label={time}
                      variant={
                        habit.time_of_day === time ||
                        (time === "Anytime" && !habit.time_of_day)
                          ? "filled"
                          : "outlined"
                      }
                      color="primary"
